...
const ENDPOINT = process.env.NEXT_PUBLIC_REACT_APP_BASE_URL as string
let socket: any
...
const Chats = () => {
    ...
    useEffect(() => {
        ...
        userid && fetchData()
    }, [messages, userid, isChat, list, query])
    ...
    const fetchData = () => {
        ...
        allChats(userid).then((data: any) => {
            ...
            if (query != "") {
                lists = lists.filter((user: any) => {

                    const usernameMatch = user.users[0]?.name
                        .toLowerCase()
                        .includes(query.toLowerCase());

                    if (usernameMatch) return user
                })
            }
            setChatList(lists)
        });
